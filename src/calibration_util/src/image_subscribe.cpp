...
image_subscribe::image_subscribe(ros::NodeHandle node):node_(node)
{
    ...
    img_sub_= it.subscribe("/tiscamera/image_raw", 1, &image_subscribe::imageCallback,this);
//    img_sub_= it.subscribe("/zedcamera/image_raw", 1, &image_subscribe::imageCallback,this);
    update_img=false;
}
