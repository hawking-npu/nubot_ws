...
public:
        ...
        //获取数据点，根据数据点反求控制点。
        void get_controlpoint(DPoint *Point_online);
        void matrix_inv(double matrix_before[NN][NN],double matrix_after[NN][NN],int n);//矩阵求逆
        double getA(double arcs[NN][NN],int n);//按第一行展开计算|A|
        void getAStart(double arcs[NN][NN],int n,double ans[NN][NN]);//计算每一行每一列的每个元素所对应的余子式，组成A*
        bool GetMatrixInverse(double src[NN][NN],int n,double des[NN][NN]);//得到给定矩阵src的逆矩阵保存到des中
        void generateCurve(DPoint *ctrlPoints, double *u);//根据控制点绘制B样条曲线
        double Nu(int i, double u2, int k, double *u);
