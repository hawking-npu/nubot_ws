...
////可能需要修改
const double update_T = 15;//15ms更新一次
const DPoint oppGoalPosr = DPoint( 890.0, -100.0);
const DPoint oppGoalPosl = DPoint( 890.0,  100.0);
const DPoint ourGoalPosr = DPoint(-890.0, -100.0);
const DPoint ourGoalPosl = DPoint(-890.0,  100.0);
const DPoint parking_point = DPoint(-800.0, 0.0);
const double LENGTH_FIX = 1.0; //场地站位等比缩小 1.0/3
