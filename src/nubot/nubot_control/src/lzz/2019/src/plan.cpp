...
void Plan::move2Positionwithobs_noball(DPoint target, float maxvel, float maxacc, bool avoid_ball, double distance_thres)
{
...
void Plan::rotate2Position(DPoint target, double maxw, double angle_thres)
{
    DPoint ori = target-robot_pos_;
    m_behaviour_.setAppvx(0.0);
    m_behaviour_.setAppvy(0.0);
    double tmp=ori.angle().radian_-robot_ori_.radian_;
    if(fabs(tmp)>angle_thres)
        m_behaviour_.rotate2AbsOrienation(1.0,0.1,ori.angle().radian_,maxw,robot_ori_.radian_);
    else
        m_behaviour_.setAppw(0);
}
