...
{
    DPoint temp;
    temp=target; temp-=_robot_pos;
    //转角度
    double maxw=max_w;
    rotate2AbsOrienation(pval, dval, temp.angle().radian_, maxw, _robot_ori);//弧度
...
void Behaviour::rotate2AbsOrienation(float pval, float dval, float orientation,float maxw,const Angle & _robot_ori)//自身坐标系
{
    ...
    while(fabs(temp)>eps)
    {
        while(fabs(temp)>=fabs(tmp))
        {
            temp-=w_now*update_T;
            delta_w=maxw-w_now;
            w_now=pval*delta_w+dval*(w_now-w_past)/update_T;
            setAppw(w_now);
            w_past=w_now;
        }
        while(fabs(temp)>=fabs(tmp))
